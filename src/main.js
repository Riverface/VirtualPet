...
$(document).ready(function () {
    ...
    var pagethread = setInterval(() => {
        ...
        if (pet.working && pet.sleepflag) {
            $("#workstatus").text("clocking out after this hour, sprinting home, then going to bed ASAP.")
        }
        if (pet.workflag && !pet.working) {
        ...
        if (!pet.sleeping) {
            ...
            $("#sleepwake").text("Wake");
            $("#workstatus").text("Sleeping");
        }
        if (pet.sleepflag && !pet.sleeping) {
            $("#workstatus").text("getting ready to sleep");
        }
